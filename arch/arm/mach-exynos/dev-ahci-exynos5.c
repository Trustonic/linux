...
static struct clk *clk_sata, *clk_sataphy, *clk_sata_i2c, *clk_sclk_sata;

static int exynos5_ahci_init(struct device *dev, void __iomem *mmio)
{
	int val, ret;

	phy_i2c_base = ioremap(EXYNOS5_PA_SATAPHY_I2C, SZ_4K);
	if (!phy_i2c_base) {
		dev_err(dev, "failed to allocate memory for SATA PHY\n");
		return -ENOMEM;
	}

	phy_ctrl = ioremap(EXYNOS5_PA_SATAPHY_CTRL, SZ_64K);
	if (!phy_ctrl) {
	...
	__raw_writel(EXYNOS5_SATA_PHY_CONTROL_ENABLE, EXYNOS5_SATA_PHY_CONTROL);
	...
	dev_err(dev, " exynos5_ahci_init FAIL\n");
...
static void exynos5_ahci_exit(struct device *dev)
{
	clk_disable(clk_sclk_sata);
	clk_put(clk_sclk_sata);
...
#ifdef CONFIG_PM
static int exynos5_ahci_suspend(struct device *dev)
{
	clk_disable(clk_sclk_sata);
	clk_disable(clk_sata_i2c);
	clk_disable(clk_sataphy);
...
static int exynos5_ahci_resume(struct device *dev)
{
	unsigned int val;
	int ret;
	...
	dev_err(dev, " exynos5_ahci_resume FAIL\n");
	return -1;
}
#endif
...
static struct ahci_platform_data exynos5_ahci_pdata = {
	...
	.exit = exynos5_ahci_exit,
#ifdef CONFIG_PM
	.suspend = exynos5_ahci_suspend,
	.resume = exynos5_ahci_resume,
#endif
};

static struct resource exynos5_ahci_resource[] = {
	[0] = DEFINE_RES_MEM(EXYNOS5_PA_SATA, SZ_64K),
	[1] = DEFINE_RES_IRQ(EXYNOS5_IRQ_SATA),
};
